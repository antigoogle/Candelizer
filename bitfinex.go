...
func BFXTrades(symbol string) TradeChannel {
	url := "https://api.bitfinex.com/v1/trades/" + symbol
	channel := make(chan Trade)
	tc := TradeChannel{Exchange: "BFX", Symbol: symbol, Channel: channel}
			...
			if err != nil {
				...
				func() {
					...
					for i := len(value) - 1; i >= 0; i-- {
						...
						trade := Trade{Date: timestamp, Price: price, Amount: amount}
